...
def host(request):
    ...
    return render(request,"host.html",{'collab_id':request.session.get('collab_id', False),'sender':request.session.get('lemail', False),'sender_name':request.session['lname']})